...
describe('TelegramReader basic', () => {
	...
	test('single random byte', () => {
		expect(telegramReader.addRawData([123])).toBe(TelegramState.pending);
	...
	test('single start byte', () => {
		const arr = [0x68];
		expect(telegramReader.addRawData(arr)).toBe(TelegramState.pending);
	...
	test('four start bytes with extra', () => {
		const len = 0x10;
		const arr = [0x68, len, len, 0x68];
		expect(telegramReader.addRawData([123, 456, ...arr])).toBe(TelegramState.pending);
		expect(telegramReader["data"]).toStrictEqual(arr);
		expect((telegramReader["data"]).length).toBe(4);
		expect(telegramReader["possibleStartFound"]).toBeTruthy();
		expect(telegramReader["currentTelegram"].lengthData).toBe(len);
...
describe('Telegram reader other example data', () => {
	...
	test('Other KAIFA MA309M https://www.gurux.fi/node/18232 1st', () => {
		...
		expect(telegramReader.addRawData(data)).toBe(TelegramState.available);
...
describe('TelegramReader complete data package', () => {
	...
	for(let i = 0; i < data.length; i++) {
		test(`single byte data[${i}] = [${data[i][0]}]`, () => {
			expect(telegramReader.addRawData(data[i])).toBe(TelegramState.pending);
			expect(telegramReader["telegrams"]).toStrictEqual([]);
		});
	}
	test('last byte', () => {
		expect(telegramReader.addRawData(last)).toBe(TelegramState.available);
		expect(telegramReader["telegrams"].length).toBe(1);
